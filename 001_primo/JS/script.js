...
function addControls(elements, typeControl, name, legend){
    let form = $("<div id='container'></div>");
    let fieldset = $("<fieldset></fieldset>").appendTo(form);
    $("<legend>"+ legend +"</legend>").appendTo(fieldset);
    if (typeControl === "radio" || typeControl === "checkBox"){
        $.each(elements, function (index, element) {
            $("<label for='" + name + "-" + index + "'>" + element + "</label>").appendTo(fieldset);
            $("<input type='" + typeControl+ "' name='" + name + "' id='" + name + "-" + index + "' value ='" + element + "'>").appendTo(fieldset).checkboxradio();
        });
        $("<input type='button' value='Print value'>").click(function () {
            $("[name=" + name + "]:checked").each(function (index, value) {
                $(".footer").append("<div class='result'>" + $(value).val() + "</div>");
            });
        }).append(form).button();
    } else if(typeControl === "datepicker"){
        $.each(elements, function (index, element) {
            $("<label for='" + name + "-" + index + "'>" + element + " Date: " + "</label>").appendTo(fieldset);
            let par = $("<p></p>").appendTo(fieldset);
            $("<input type='text' name='" + name + "' id='" + name + "-" + index + "'>").appendTo(par).datepicker({"dateFormat":"dd/m/yy"});
...
function createTabs(data, parent, e){
    ...
    if (tabs.tabs("instance")){
        ...
        $.each(data["Labels"], function(index, element){
            ...
            $("<div id='tabs-" + index + "'></div>").appendTo(tabs).append(addControls(data["Elements"], data["TypeControl"],));
        });
        tabs.tabs();
